...
CSS3全屏动画插件 ， 使用方法：
	...
	HJL.E.particle( //启动粒子发射器
		'.X_rain' ,//要复制的粒子对象，CSS选择器
		[ 1400 ,2500 ] , // 数组[0] = 粒子容器的运行速度（毫秒） ， 数组[1] = 速度随机加成 （毫秒）
		"100", //容器的运行距离，为屏幕的宽度（百分比） , 0 = 不运动 ， -1 = 随机距离（最大不会超过屏幕宽度）
		40 , //容器和粒子生成速度（毫秒）
		45 , //容器发射的角度（正负数），超过正负360度后，会减去360，剩余的会进行随机角度。
			  //【参考角度：0=右，45=右下，90=直下，135左下，180=左，225左上，270直上，315右上。负数反之。】
		[ '0%{transform:rotate(180deg);}100%{transform:rotate(310deg);}' , '1s' ] , 
		...
		[ '30%' , '70%' , '30%' , '70%'  ] ,  // 容器出生的范围left ，right ， top ， bottom ， 单位 % ，参考屏幕宽度高度（0-150%）
		'ease-in ', //容器的动画附加属性
...
var HJL  = {
	...
	bl:{
		...
		ani_li_speed_random: 0 , //板块里面对象的动画是否加上随机速度，  如果是 0  表示不使用； 如果使用，建议设置在1-1000之间。 
		...
		function set(  tag , ani , speed ,pause  ){ //执行动画
			...
			$(tag).each(function(i, e) { //逐个给对象添加动画
				...
				if(ani.indexOf("~")>-1){ //如果使用的是Amation的动画库
						...
						if(ty=="in"){
							HJL.css3_set(	TAG , "animation" , ani + "  "+ end_speed  +"s   "+ pause*0.001+"s" ); //CSS3兼容处理
							setTimeout( function(){ 
									 //动画完毕后清除动画
									//TAG.css({"animation":""});TAG.css({"-webkit-animation":""});TAG.css({"-moz-animation":""});
									//TAG.css({"-ms-animation":""});TAG.css({"-o-animation":""});
							...
							//$(this).css({"animation":ani + " forwards "+ end_speed  +"s   "+ pause*0.001  +"s" }); 
 							HJL.css3_set(	TAG, "animation" , ani + " forwards "+ end_speed  +"s  "+ pause*0.001+"s"); //CSS3兼容处理
						}
					//	//处理一次性动画
					/*
	...
	E:{ //特效系列
		...
		//17个参数
			...
			HJL.E.b.Intval_rain = setInterval( function(){
					...
					/*飘雪图层*/
					Tag_div_1.css({"overflow":"visible","position":"absolute","display":"block","z-index":15 });
		...
		zoom:{ //图片点击放大插件 ------------------------
			...
			setMove_touch:function(IMG){ //手指拖动、缩放事件
				...
				EML.on('pan', function(ev) {
						...
						if( ev.isFinal ){	ys_x = 	end_x ; ys_y = 	end_y ; }
						//console.log(	end_x + "-"+ys_x	);
				});
				...
				var ys_scale = HJL.E.zoom.B.bimg_scale ;	 //原始缩放值			
				var Timmer;				
				EML.on('pinch', function(ev) {	
						if(ev.pointerType == "mouse" ){return false;} //  禁止鼠标执行代码
						
						pan_IO = 0 ; //关闭拖动				
						var end_scale = ys_scale + ev.scale -1   ;
						
						HJL.css3_set( IMG , "transform" , "scale("+end_scale*end_scale+","+end_scale*end_scale+")" );
						...
						//ratioDelta = !delta ? 1 + 0.1 : 1 - 0.1,
						HJL.E.zoom.setBFB( 1.1  ,  HJL.E.zoom.B.bimg_endw*end_scale   , 0 ); //设定显示缩放百分比
						//$("#debug").html(	 IMG.width() 	)
